...
def render_visualization(viz_type, df):
    if viz_type == "Pie Chart":
        ...
        st.title("3D Scatter Plot: Age, Billing Amount, and Length of Stay")
        st.write("This interactive visualization displays the relationship between Age, Billing Amount, and Length of Stay.")
        df['Length of Stay'] = (pd.to_datetime(df['Discharge Date']) - pd.to_datetime(df['Date of Admission'])).dt.days



        # Ensure required columns exist
        required_columns = ['Age', 'Billing Amount', 'Length of Stay']
    
        fig = plt.figure(figsize=(12, 8))
        ax = fig.add_subplot(111, projection='3d')
            
            # Scatter plot
        sc = ax.scatter(df['Age'], df['Billing Amount'], df['Length of Stay'], 
        c=df['Length of Stay'], cmap='viridis', alpha=0.8)
            
            # Labels and title
        ax.set_title("3D Scatter Plot: Age vs Billing Amount vs LOS", fontsize=16)
        ax.set_xlabel("Age", fontsize=12)
        ax.set_ylabel("Billing Amount (£)", fontsize=12)
        ax.set_zlabel("Length of Stay (Days)", fontsize=12)

            # Add color bar
        cbar = plt.colorbar(sc, ax=ax, pad=0.1)
        cbar.set_label('Length of Stay (Days)', fontsize=12)
...
if selected_main_page == "Overview":
    ...
    # Optionally, you can add more interactive features like parameter sliders, dropdowns, etc.
        ...
        plt.ylabel('Count')
        plt.legend(title='Medical Condition')
        st.write("### Monthly Trends in Admissions by Medical condition")
